...
aca <- 
  readRDS(file = "data/AllCarerAssessments.rds")
...
a3 <-
  ...
  select(person_ref:jca, index_date, year, 
         time.since.first.flag, number.of.carer.flags, overall) 

# check
glimpse(a3)
a3 %>% 
  # count(person_ref, sort = T)
  count(person_ref, year, sort = T)
...
SummariseDemographics(df = a3, grp = grp_vars[c(2,3)], summ = summ_vars[1], yearly = T) %>% 
  pull(n.person_ref) %>% 
  sum(.)
245/1580
...
demo <-
  ...
  rownames_to_column() %>% 
  mutate(nms = paste("T3", rowname, Var1, sep = "_"),
         summ = map(.x = Var1, 
                    .f = ~SummariseDemographics(df = a3, 
                                                grp = .x, 
                                                summ = summ_vars, 
                                                yearly = T)
         ),  
         std = map(.x = Var1,
                   .f = ~StandardiseDemographics(df = pop, grp = .x, summ = n, yearly = T)),
...
# check
a3_out_demo$T2_1_overall %>%
  pull(number.carers) %>%
  sum(.)

a3_out_demo$T2_2_age_band %>%
  pull(number.carers) %>%
  sum(.)
...
int <-
  ...
  std = map(.x = Var1,
            .f = ~StandardiseDemographics(df = pop,
                                          grp = .x, 
                                          summ = n,
                                          yearly = T)
            ),
...
a3_out_int$T2_5_age_band_gender %>%
...
# collect summaries
a3_out <-
  append(a3_out_demo, a3_out_int)
# names(a3_out)

a3_out$T2_5_age_band_gender
# zeros dropped
...
a3_out_demo_plots <- 
  ...
  set_names(x = ., nm = names(a3_out_demo))

a3_out_demo_plots$T2_3_gender
...
a3_out_int_plots <- 
  ...
  set_names(x = ., nm = names(a3_out_int))

a3_out_int_plots$T2_5_age_band_gender

a3_int_fig <- 
  cowplot::plot_grid(plotlist = a3_out_int_plots, 
                     # labels = names(a3_out_demo_plots),
                     # label_size = 10, 
                     # label_x = -0.05,
                     # label_y = 0.05,
                     # hjust = -0.8, 
...
a3_plots <- 
  append(a3_out_demo_plots, a3_out_int_plots)


# a3 misc -----------------------------------------------------------------
## time from first carer flag
a3 %>% count(person_ref, year, sort = T)
