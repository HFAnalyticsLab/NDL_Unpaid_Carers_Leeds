...
a1_gp <- 
  readRDS(file = "data/a1_out_gp.rds")

a2_gp <- 
  readRDS(file = "data/a2_out_gp.rds")
...
# age
range(a1_gp$age, na.rm = T)

# date
range(a1_gp$gp_date, na.rm = T)


# eda ---------------------------------------------------------------------
glimpse(a1_gp)

a1_gp %>% count(nhs_number, sort = T)
...
SummariseDemographics(df = a1, grp = grp_vars[4], summ = summ_vars[1], yearly = T)
...
demo <-
  ...
  rownames_to_column() %>% 
  mutate(nms = paste("T1", rowname, Var1, "gp", sep = "_"),
         summ = map(.x = Var1, 
...
# check
# a1_out_demo_gp$T1_1_overall %>%
#   pull(number.carers) %>%
#   sum(.)
# 
# a1_out_demo_gp$T1_2_age_band %>%
#   pull(number.carers) %>%
#   sum(.)
...
int <- 
  ...
  as_tibble() %>% 
  rownames_to_column() %>% 
  mutate(nms = paste("T1", as.integer(rowname)+4, Var1, Var2, "gp", sep = "_"))
...
# a1_out_int_gp$T1_5_age_band_gender %>%
...
# collect summaries
a1_out_gp <-
  append(a1_out_demo_gp, a1_out_int_gp)
# names(a1_out)
...
a1_out_demo_gp_plots <- 
  ...
  set_names(x = ., nm = names(a1_out_demo_gp))

# a1_out_demo_gp_plots$T1_2_age_band
...
a1_out_int_gp_plots <- 
  ...
  set_names(x = ., nm = names(a1_out_int_gp))

# a1_out_gp$T1_4_imd_decile %>%
#   pull(number.carers) %>%
#   sum(.)

# a1_out_int_gp_plots$T1_5_age_band_gender
# a1_out_int_gp_plots$T1_6_age_band_imd_decile
# a1_out_int_gp_plots$T1_7_gender_imd_decile
...
a1_plots_gp <- 
  append(a1_out_demo_gp_plots, a1_out_int_gp_plots)

# a1_plots_gp$T1_5_age_band_gender
# a1_plots_gp$T1_6_age_band_imd_decile
# a1_plots_gp$T1_7_gender_imd_decile
...
# a1 %>% count(person_ref)
# a1_plots_gp$T1_5_age_band_gender
# a1_plots_gp$T1_6_age_band_imd_decile_gp
# a1_plots_gp$T1_7_gender_imd_decile
